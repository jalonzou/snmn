...
class BatchLoaderClevr:
    ...
    def load_one_batch(self, sample_ids):
        ...
        # Jiangnan: image understanding
        if self.load_question:
            question_batch = [''] * actual_batch_size
        # Jiangnan: question understanding
        if self.load_sent_percent:
            sent_percent_batch = [0 for i in range(actual_batch_size)]
        ...
        for n in range(len(sample_ids)):
            ...
            # Jiangnan: image understanding
            if self.load_question:
                question_batch[n] = iminfo['question_str']
            # Jiangnan: question understanding
            if self.load_sent_percent:
                sent_percent_batch[n] = iminfo['sent_percent']
        ...
        # Jiangnan: image understanding
        if self.load_question:
            batch['question_batch'] = question_batch
        # Jiangnan: question understanding
        if self.load_sent_percent:
            batch['sent_percent_batch'] = sent_percent_batch
