...
cfg.TEST.SPLIT_VQA = cfg.TEST.SPLIT_VQA.split('_')[0]
...
for n_batch, batch in enumerate(data_reader.batches()):
    ...
    # Jiangnan: image understanding
    if 'question_batch' in batch:
        res_bool = vqa_predictions == vqa_labels
        question_batch = batch['question_batch']
        for qid in range(len(question_batch)):
            cur_sent = question_batch[qid]
            if question_batch[qid] not in acc_on_qtype:
                acc_on_qtype[cur_sent] = [0, 0]
            acc_on_qtype[cur_sent][0] += res_bool[qid]
            acc_on_qtype[cur_sent][1] += 1
    
    # Jiangnan: question understanding
    if 'sent_percent_batch' in batch:
        res_bool = vqa_predictions == vqa_labels
        sent_percent_batch = batch['sent_percent_batch']
        for sid in range(len(sent_percent_batch)):
            cur_percent = sent_percent_batch[sid]
            if cur_percent not in acc_on_wtype:
                acc_on_wtype[cur_percent] = [0, 0]
            acc_on_wtype[cur_percent][0] += res_bool[sid]
            acc_on_wtype[cur_percent][1] += 1
...
# Jiangnan: image understanding
if acc_on_qtype:
    with open(os.path.join(result_dir, 'image_understanding_results_%s.txt' % cfg.TEST.SPLIT_VQA), 'w') as f:
        for i, q in enumerate(acc_on_qtype.keys()):
            f.write('%d , %s , %.2f\n' % (i, q, float(acc_on_qtype[q][0]) / float(acc_on_qtype[q][1])))

# Jiangnan: question understanding
if acc_on_wtype:
    with open(os.path.join(result_dir, 'question_understanding_results_%s.txt' % cfg.TEST.SPLIT_VQA), 'w') as f:
        for i, q in enumerate(acc_on_wtype.keys()):
            f.write('%d , %d , %.2f\n' % (i, q, float(acc_on_wtype[q][0]) / float(acc_on_wtype[q][1])))
